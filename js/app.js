...
---------------------------------------------------------------------- */
	...
	function getEventDetail(slug, search){
		...
		$.getJSON('php/event_detail.php', {slug:slug}, function(d){
			...
			//$('div.detail p.breadcrumbs span').html(d.title);
		...
		$('body').on('mouseover', 'div.event', function(){
			if( hover == 'hoverable' ){		
				...
				var pointer = $('canvas.pointer'),
		    		...
		    		//clientHalfHeight = ((clientHeight-footerHeight) / 2) - (dateBarHeight / 2),
		     		//pointerDirection = ((eventTop - currentTop) > clientHalfHeight) ? 'bottom' : 'top',
		     		//pHeightType = (eventType == 'major') ? 80 : 30,
		     		//pHeightTop = clientHalfHeight - ((eventTop - currentTop) + pHeightType) - 0,
		     		//pHeightBottom = eventTop - clientHalfHeight - dateBarHeight - currentTop + 10,
		     		//pointerHeight = (pointerDirection == 'bottom') ? pHeightBottom : pHeightTop;
		     		//pointerHeight = (pointerHeight < 14) ? 0 : pointerHeight;
				...
				// Set canvas position
				//var eventTopAdjust = (pointerDirection == 'top') ? eventTop + pHeightType : eventTop - pointerHeight + 10;
	...
	function showTooltip(message){
		if(tooltipFlag){ // If tooltip is currently visible
			...
			tooltipShow = setTimeout(function(){ 
				//$('div.tooltip').html(message).css('opacity',1); 
...
/*
			...
			$.getJSON('php/get_favorites.php', {'user_id':user_id}, function(d){
				// Update count in dropdown
				$('div.favorites h4').html('My Favorites ('+d.length+')');
...
---------------------------------------------------------------------- */
	...
	$('div.footer div.period').on(click, function(){
		...
		//setTimeout(function(){ animateSidebar = true; }, 300);
	});
